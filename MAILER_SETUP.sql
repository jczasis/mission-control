-- ============================================================================
-- INFINITY MAILER - Schema Supabase
-- Tablas para gestionar campañas de email con aprobación y auditoría
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS public.email_campaigns (
  ...
  body TEXT NOT NULL,
  html_body TEXT,
  status TEXT NOT NULL DEFAULT 'draft', -- draft, pending, approved, scheduled, sent, failed
  ...
  -- Campos de validación (de mailing-infinity agent)
  validated BOOLEAN DEFAULT FALSE,
  validation_result JSONB DEFAULT '{}', -- {passed: true, warnings: [], errors: []}
  preflight_report TEXT, -- HTML report de validación
  ...
  -- Integrations
  zoho_campaign_id TEXT, -- ID en Zoho Mail si se enviró realmente
  is_infinity_box BOOLEAN DEFAULT TRUE, -- Siempre true para Infinity Box
  -- Metadata
  notes TEXT,
...
-- Almacena destinatarios y su estado individual por campaña
CREATE TABLE IF NOT EXISTS public.email_recipients (
  ...
  name TEXT,
  company TEXT,
  lead_source TEXT, -- seia, manual, infinity_import, etc
...
-- Log de todas las acciones (auditoria y troubleshooting)
CREATE TABLE IF NOT EXISTS public.email_history (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  campaign_id UUID NOT NULL REFERENCES public.email_campaigns(id) ON DELETE CASCADE,
  recipient_id UUID REFERENCES public.email_recipients(id) ON DELETE SET NULL,
  action TEXT NOT NULL, -- created, approved, rejected, scheduled, sent, failed, bounced, opened, clicked, unsubscribed
  actor TEXT NOT NULL,
  details TEXT,
  error_code TEXT,
  error_message TEXT,
...
-- Templates reutilizables para campañas
CREATE TABLE IF NOT EXISTS public.email_templates (
  ...
  category TEXT, -- seia, infinity_box, versasteel, general
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT now()
);

-- 5. TABLA: outreach_blacklist (de mailing-infinity agent)
-- Tracking de bounces y blacklist de emails
CREATE TABLE IF NOT EXISTS public.outreach_blacklist (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  email TEXT UNIQUE,
  reason TEXT NOT NULL, -- hard_bounce, soft_bounce_3x, generic_email, govt, test_domain, manual_block
  bounce_code TEXT, -- 5.1.1, 5.4.4, etc
  bounce_count INT DEFAULT 0,
...
-- 6. TABLA: outreach_bounces (tracking de bounces)
CREATE TABLE IF NOT EXISTS public.outreach_bounces (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  campaign_id UUID NOT NULL REFERENCES public.email_campaigns(id) ON DELETE CASCADE,
  recipient_id UUID REFERENCES public.email_recipients(id) ON DELETE SET NULL,
  email TEXT NOT NULL,
  bounce_code TEXT NOT NULL, -- 5.1.1, 5.4.4, etc
  ...
  final_status TEXT, -- blacklisted, will_retry, manual_review
  metadata JSONB DEFAULT '{}'
);

-- 7. TABLA: sending_config (reglas globales)
CREATE TABLE IF NOT EXISTS public.sending_config (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  config_key TEXT NOT NULL UNIQUE, -- maxPerDomain, maxPerDay, delayMin, etc
...
-- Insert default sending config (de MAILER_RULES_CONFIG.json)
INSERT INTO public.sending_config (config_key, config_value, data_type, description) VALUES
...
CREATE INDEX idx_email_history_action ON public.email_history(action);
-- Índices para outreach_blacklist y bounce tracking
CREATE INDEX idx_outreach_blacklist_email ON public.outreach_blacklist(email);
CREATE INDEX idx_outreach_bounces_campaign_id ON public.outreach_bounces(campaign_id);
CREATE INDEX idx_outreach_bounces_email ON public.outreach_bounces(email);
CREATE INDEX idx_outreach_bounces_bounce_type ON public.outreach_bounces(bounce_type);

-- ============================================================================
-- VISTAS para frontend
-- ============================================================================

-- Vista: resumen de campaña (para tarjeta en dashboard)
...
-- Vista: emails pendientes para mañana
...
-- Vista: stats globales del mailer
...
-- ============================================================================
-- RLS (Row Level Security) - TODO: Activar cuando tengas auth
-- ============================================================================
